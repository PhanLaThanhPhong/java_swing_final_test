...
//        CreateInvoiceGUI createInvoiceGUI = new CreateInvoiceGUI();
        ...
        if(type == Invoice.InvoiceType.EXPORT){
            for (InvoiceDetail invoiceDetail : listInvoiceDetail) {
                ...
                createInvoiceGUI.listProductInvoiceModel.addRow(new Object[]{
                        invoiceDetail.getProductId(),
                        product.getName(),
                        invoiceDetail.getQuantity(),
                        invoiceDetail.getPrice(),
                        invoiceDetail.getPrice() * invoiceDetail.getQuantity()
                });
            }

            int computerIdIndex = invoiceIsSelect.getComputerId() - 1;
            if (computerIdIndex >= 0 && computerIdIndex < createInvoiceGUI.listComputerID.getItemCount()) {
                createInvoiceGUI.listComputerID.setSelectedIndex(computerIdIndex);
            } else {
                createInvoiceGUI.listComputerID.setSelectedItem(null); // hoặc giá trị mặc định
            }

            if (invoiceIsSelect.getCreatedToAccountId() == null) {
                createInvoiceGUI.listAccountID.setSelectedIndex(0); // khách hàng vãng lai
            } else {
                int accountIdIndex = invoiceIsSelect.getCreatedToAccountId();
                if (accountIdIndex >= 0 && accountIdIndex < createInvoiceGUI.listAccountID.getItemCount()) {
                    createInvoiceGUI.listAccountID.setSelectedIndex(accountIdIndex);
                } else {
                    createInvoiceGUI.listAccountID.setSelectedItem(null); // hoặc giá trị mặc định
                }
            }
        } else {
            for (InvoiceDetail invoiceDetail : listInvoiceDetail) {
...
// Đổ dữ liệu lên
        ...
        int employeeIdIndex = invoiceIsSelect.getCreatedBy() - 1;
        if (employeeIdIndex >= 0 && employeeIdIndex < createInvoiceGUI.listEmployeeID.getItemCount()) {
            createInvoiceGUI.listEmployeeID.setSelectedIndex(employeeIdIndex);
        } else {
            createInvoiceGUI.listEmployeeID.setSelectedItem(null); // hoặc giá trị mặc định
        }

// nếu isPaid là true tức là hóa đơn đã được thanh toán rồi
        ...
        createInvoiceGUI.idInvoiceSelected.setText(String.valueOf(idInvoiceIsSelected)); // lấy id của hóa đơn muốn edit để lưu vào label
